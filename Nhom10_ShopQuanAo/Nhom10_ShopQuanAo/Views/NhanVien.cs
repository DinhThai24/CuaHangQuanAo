    ...
    public partial class NhanVien : Form
    {
        ...
        private void btnThem_Click(object sender, EventArgs e)
        {
            if(ktTrungTK(txtTK.Text))
            {
                ...
                if (ktTrungMNV(txtMaNV.Text))
                {
                    ...
                    string chuoiTV = "Insert into NhanVien values ('" + txtMaNV.Text + "', N'" + txtTenNV.Text + "','" + txtSDT.Text + "','" + txtEmail.Text + "','"+txtTK.Text+"')";
        ...
        private void dgvNV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && e.RowIndex < dgvNV.Rows.Count)
            {
                ...
                txtEmail.Text = GetValueFromCell(row, 3);
                txtTK.Text = GetValueFromCell(row, 4);
            }
        }
                    ...
                    if(ktTrungTK(txtTK.Text))
                    {
                        ...
                        if (kqTK == 1)
                        {
                            if (ktTrungMNV(txtMaNV.Text))
                            {
                                ...
                                string chuoiNV = "Insert into NhanVien values ('" + txtMaNV.Text + "', N'" + txtTenNV.Text + "','" + txtSDT.Text + "','" + txtEmail.Text + "','" + txtTK.Text + "')";
                                int kqNV = db.getNonQuery(chuoiNV);

                                if (kqNV == 1)
                                {
                                    MessageBox.Show("Đã Sửa", "Thông báo");
                                    load_grid(); // Tải lại dữ liệu lên grid sau khi cập nhật thành công
                                }
                                else
                                {
